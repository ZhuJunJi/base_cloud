...
public class BaseWorkItemDataServiceImpl implements BaseWorkItemDataService {
    ...
    private FieldHandler getFieldHandler(String collection, String fieldKey){
        ...
        FieldHandler fieldHandler = SpringBeanUtil.getBeanIfAvailable(fieldHandlerName, FieldHandler.class);

        if(fieldHandler != null){
            return fieldHandler;
        }
        // 没有实现指定的字段处理器时获取默认的字段处理器
        FieldHandler defaultFieldHandler = SpringBeanUtil.getBeanIfAvailable(
                GlobalConstants.DEFAULT_FIELD_HANDLER,
                FieldHandler.class);
